...
tall = [random.randint(0, 1000) for x in range(1000)]

# Tid for jens-sortering
tid_jens = timeit.timeit(lambda: jens_sort(tall.copy()), number=1000)
print(f"Jens-sort tid: {tid_jens}")

# Tid for boblesortering
tid_bubble = timeit.timeit(lambda: bubble_sort(tall.copy()), number=1000)
print(f"Boblesortering tid: {tid_bubble}")

# Tid for utvalgssortering
tid_selection = timeit.timeit(lambda: selection_sort(tall.copy()), number=1000)
print(f"Utvalgssortering tid: {tid_selection}")

# Tid for quicksort
tid_quicksort = timeit.timeit(lambda: quicksort(tall.copy()), number=1000)
